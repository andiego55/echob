/**
 * /student/cases/:id/hypotheses/:hypId — Hypothesen-Dialog (student-scoped).
 * Geführtes Echo-Gespräch zu einer Arbeitshypothese (tastend, keine Diagnose).
 * Echo kennt den vollen Fallkontext inkl. gespeicherter Hypothesen.
 */
...
export default function StudentHypothesisDialogPage() {
  const { id, hypId } = useParams<{ id: string; hypId: string }>()
  ...
  const startTrigger = `__${hypId}_start__`

  const { data: history = [], isSuccess: historyLoaded } = useQuery({
    queryKey: ['student-hyp-history', id, hypId],
    queryFn: () => studentApi.hypHistory(id!, hypId!),
    enabled: !!id && !!hypId && !!hyp,
    refetchOnWindowFocus: false,
    retry: false,
  })

  const chat = useMutation({
    mutationFn: (message: string) => studentApi.hypChat(id!, hypId!, message),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['student-hyp-history', id, hypId] })
      setInput('')
      setPending(null)
    },
  ...
  useEffect(() => { startedRef.current = false }, [hypId])

  useEffect(() => {
    if (!historyLoaded || startedRef.current || !hyp) return
    startedRef.current = true
    const alreadyStarted = (history as StudentEchoMessage[]).some(m => m.content === startTrigger || m.role === 'assistant')
    if (!alreadyStarted) chat.mutate(startTrigger)
  ...
  const summaryMutation = useMutation({
    mutationFn: () => studentApi.hypGenerate(id!, hypId!),
    onSuccess: (data) => { setSummary(data.summary); setSavedSummary(false) },
  })
  const saveSummaryMutation = useMutation({
    mutationFn: () => studentApi.hypSave(id!, hypId!, summary!),
    onSuccess: () => { setSavedSummary(true); qc.invalidateQueries({ queryKey: ['student-hypotheses', id] }) },
  })
  const resetMutation = useMutation({
    mutationFn: () => studentApi.hypReset(id!, hypId!),
    onSuccess: () => {
      setSummary(null)
  ...
  const visibleMessages = hyp ? (history as StudentEchoMessage[]).filter(m => m.content !== startTrigger) : []
      ...
      <div className="flex flex-col" style={{ height: 'calc(100vh - 56px - 49px)' }}>
        ...
        <div className="border-b border-brand-border bg-white px-6 py-3 flex items-center justify-between gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={() => navigate(`/student/cases/${id}/hypotheses`)} className="text-xs text-brand-muted hover:text-navy transition-colors ml-2">
              ← Zurück
            </button>
          </div>
        </div>
          ...
          <div className="mx-auto max-w-[780px] px-6 py-6 space-y-4">
            ...
            {visibleMessages.map((msg) => (
              <ChatMessage
                ...
                safetyLevel={msg.role === 'assistant' ? msg.safety_level : undefined}
              />
            ))}
          ...
          {showExamples && hyp.introQuestions.length > 0 && (
            <div className="mx-auto max-w-[780px] mb-2 rounded-2xl border border-brand-border bg-white shadow-[0_4px_24px_rgba(15,30,46,0.08)] px-5 py-4">
              ...
              <p className="text-xs text-brand-muted mb-3">
                Noch unsicher beim Thema? Klick eine Frage – Echo erklärt sie dir, bevor ihr gemeinsam auf den Fall schaut.
              </p>
