/**
 * /student/cases/:id/echo — Echo-Dialog der/des Studierenden (session-basiert).
 * Mehrere Gespräche (umbenennbar/löschbar), Glossar, voller Fallkontext.
 */
...
const SUGGESTIONS = [
  'Welche Muster fallen dir in diesem Fall auf?',
  'Welche Szenen sind besonders aufschlussreich?',
  'Welche Fragen wären im Erstgespräch hilfreich?',
  'Worauf sollte man vorsichtig eingehen?',
]
...
export default function StudentEchoPage() {
  ...
  const { data: sessions = [] } = useQuery({
    queryKey: ['student-echo-sessions', id],
    queryFn: () => studentApi.echoSessions(id!),
    enabled: !!id,
  })
  const { data: glossary = [] } = useQuery({ queryKey: ['student-glossary'], queryFn: () => studentApi.glossary() })
  const { data: history } = useQuery({
    queryKey: ['student-echo-history', id, activeSession],
    queryFn: () => studentApi.echoHistory(id!, activeSession!),
  ...
  const chat = useMutation({
    mutationFn: (vars: { message: string; session_id?: string; thread_type?: 'topic' | 'glossary'; glossary_slug?: string }) =>
      studentApi.echoChat(id!, vars),
    onSuccess: (res) => {
      setActiveSession(res.session_id)
      setMessages(prev => [...prev, res.user_message, res.assistant_message])
      setPending(null)
      qc.invalidateQueries({ queryKey: ['student-echo-sessions', id] })
    },
    onError: () => setPending(null),
  })

  const rename = useMutation({
    mutationFn: ({ sid, title }: { sid: string; title: string }) => studentApi.echoSessionRename(id!, sid, title),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['student-echo-sessions', id] }); setEditingId(null) },
  })
  const del = useMutation({
    mutationFn: (sid: string) => studentApi.echoSessionDelete(id!, sid),
    onSuccess: (_d, sid) => {
      qc.invalidateQueries({ queryKey: ['student-echo-sessions', id] })
  ...
  const glossaryClick = (g: GlossaryTerm) => {
    ...
    chat.mutate({ message: `Bitte besprich den Begriff „${g.term}" im Kontext dieses Falls.`, thread_type: 'glossary', glossary_slug: g.slug })
  }
    ...
    <StudentShell>
      ...
      <div className="mx-auto max-w-[1100px] px-6 py-6">
        <div className="mb-4 flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h1 className="text-xl font-bold text-navy">Echo – Fallgespräch</h1>
            <p className="text-xs text-brand-muted">Echo kennt den vollen Fallkontext. Übungsraum, keine echten Patient:innen.</p>
          </div>
        ...
        {showGlossary && glossary.length > 0 && (
          <div className="mb-4 card">
            <p className="mb-2 text-xs text-brand-muted">Begriff anklicken, um ihn mit Echo im Kontext dieses Falls zu besprechen.</p>
          ...
          <div className="min-w-0 flex-1">
            <div className="card flex h-[calc(100vh-16rem)] min-h-[420px] flex-col">
                  ...
                  <div className="text-sm text-brand-muted">
                    ...
                    <div className="flex flex-col gap-2">
                      {SUGGESTIONS.map(q => (
                        <button
                          key={q}
                          onClick={() => { setPending(q); chat.mutate({ message: q, session_id: activeSession ?? undefined }) }}
                          className="rounded-brand border border-brand-border px-3 py-2 text-left text-xs transition-colors hover:border-accent hover:text-accent"
                        >
                          {q}
                        </button>
                      ))}
              ...
              <div className="mt-3 border-t border-brand-border pt-3">
                <ChatComposer value={input} onChange={setInput} onSend={send} pending={chat.isPending} placeholder="Schreibe Echo …" />
              </div>
            </div>
          </div>
        </div>
