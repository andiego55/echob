 ...
 *
 * **Drei Stapel, und die Reihenfolge ist eine Aussage.** Oben die Entwürfe, weil sie eine
 * offene Frage sind und nicht liegenbleiben sollen. In der Mitte die bestätigten — das,
 * was gilt. Unten und eingeklappt die überholten: Sie sind nicht weg, weil die Bewegung
 * selbst etwas sagt. Wer nach einem Jahr sieht, dass „Wenn ich Nein sage, bin ich
 * egoistisch" überholt ist, sieht mehr als in jeder Kurve.
 *
 * **Bestätigt heißt nicht wahr.** Deshalb steht neben jedem bestätigten Satz, wie alt die
 * Zustimmung ist, und deshalb gibt es „überholt" neben „löschen". Ein Satz, der sich
 * überlebt hat, soll stehen bleiben dürfen; einer, den jemand in einem schlechten Moment
 * geschrieben hat, muss verschwinden können.
 */
...
export default function KompassSaetzePage() {
  ...
  const stapel = useMemo(() => {
    const alle = saetze ?? []
    return {
      entwurf: alle.filter(s => s.stand === 'entwurf'),
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-[720px] px-6 py-8">
        ...
        <Fehlermeldung error={anlegen.error ?? aendern.error ?? loeschen.error} className="mt-4" />
      </div>
    </AppShell>
  )
}
