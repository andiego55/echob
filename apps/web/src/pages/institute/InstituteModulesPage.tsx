/**
 * /institute/modules — Lernmodule: eigene Module erstellen/verwalten + Marktplatz.
 * Tabs via ?view=mine|market (Standard: mine). Vereint die frühere „Marktplatz“-Seite
 * (MarketplaceView) mit der Modulverwaltung.
 */
...
import { MarketplaceView } from './InstituteMarketplacePage'
...
export default function InstituteModulesPage() {
  ...
  const view = params.get('view') === 'market' ? 'market' : 'mine'
  const setView = (v: 'mine' | 'market') => setParams(v === 'mine' ? {} : { view: v }, { replace: true })

  return (
    <InstituteShell>
      <div className="mx-auto max-w-[1100px] px-6 py-8">
        <header className="mb-5">
          <h1 className="text-2xl font-bold text-navy">Lernmodule</h1>
          <p className="mt-1 max-w-2xl text-sm text-brand-muted">
            Strukturierte Lerneinheiten aus Lektionen, Fällen, Aufgaben und didaktischem Leitfaden – zuweisbar an deine
            Studierenden. Fertige Module anderer Institute findest du im Marktplatz.
          </p>
        </header>

        <InstituteTabs
          tabs={[{ key: 'mine', label: 'Meine Module' }, { key: 'market', label: 'Marktplatz' }]}
          active={view}
          onChange={(k) => setView(k as 'mine' | 'market')}
        />

        <div className="mt-6">{view === 'mine' ? <MyModulesView /> : <MarketplaceView />}</div>
      </div>
    </InstituteShell>
  )
}
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-end">
        {!creating && <button onClick={() => setCreating(true)} className="btn-primary !py-2 !px-4 !text-sm">+ Neues Modul</button>}
      </div>
