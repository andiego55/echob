...
import type { LearningModuleDetail, ModuleStep } from '@/types'
  ...
  return (
    <InstituteShell>
      <div className="mx-auto max-w-[820px] px-6 py-8 space-y-6">
        ...
        <StepsCard moduleId={id!} steps={data.steps} />
...
function StepsCard({ moduleId, steps }: { moduleId: string; steps: ModuleStep[] }) {
  const qc = useQueryClient()
  const [adding, setAdding] = useState(false)
  const invalidate = () => qc.invalidateQueries({ queryKey: ['institute-module', moduleId] })

  const add = useMutation({
    mutationFn: (data: { title: string; content: string }) => instituteApi.moduleStepAdd(moduleId, data),
  ...
  return (
    <section className="card">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-sm font-bold text-navy">Lektionen</h2>
        {!adding && <button onClick={() => setAdding(true)} className="text-xs font-semibold text-accent hover:underline">+ Lektion</button>}
      </div>

      {adding && <StepForm onSubmit={(d) => add.mutate(d)} onCancel={() => setAdding(false)} pending={add.isPending} />}

      {steps.length === 0 && !adding ? (
        <p className="text-sm text-brand-muted">Noch keine Lektionen.</p>
      ) : (
...
function StepRow({ moduleId, step, index, total, onMove }: {
  ...
  const upd = useMutation({
    mutationFn: (d: { title: string; content: string }) => instituteApi.moduleStepUpdate(moduleId, step.id, d),
  ...
  if (editing) {
    return (
      <div className="rounded-brand border border-accent/40 bg-brand-bg p-3">
        <StepForm initial={step} onSubmit={(d) => upd.mutate(d)} onCancel={() => setEditing(false)} pending={upd.isPending} />
      </div>
    )
  }
    ...
    <div className="rounded-brand border border-brand-border bg-white px-3 py-2.5">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <button onClick={() => { if (window.confirm('Lektion löschen?')) del.mutate() }} className="ml-1 text-xs text-brand-muted hover:text-red-600">Löschen</button>
        </div>
      </div>
    </div>
  )
...
function StepForm({ initial, onSubmit, onCancel, pending }: {
  initial?: ModuleStep; onSubmit: (d: { title: string; content: string }) => void; onCancel: () => void; pending: boolean
}) {
  const [title, setTitle] = useState(initial?.title ?? '')
  const [content, setContent] = useState(initial?.content ?? '')
  return (
    <div className="space-y-2">
      <input value={title} onChange={e => setTitle(e.target.value)} maxLength={200} placeholder="Titel der Lektion"
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => onSubmit({ title: title.trim(), content })} disabled={!title.trim() || pending} className="btn-primary !py-1.5 !px-4 !text-sm">
