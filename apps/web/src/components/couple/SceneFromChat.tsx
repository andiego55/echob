...
import AnkerFall, { fallName } from './AnkerFall'
...
export default function SceneFromChat({
  ...
  // Nur fuer den Namen im Knopf. Dieselbe Abfrage benutzt AnkerFall — react-query
  // holt sie einmal.
  const faelle = useQuery({ queryKey: ['cases'], queryFn: casesApi.list })
  const zielFall = (faelle.data?.cases ?? []).find(c => c.id === caseId)
  ...
  if (!caseId) {
    return <AnkerFall coupleId={coupleId} caseId={null} titel="Szene erstellen" />
  }
    ...
    <div>
      ...
      >
        ...
        <p className="mt-0.5 text-[0.72rem] leading-snug text-brand-muted">
          {zielFall
            ? `Echo macht daraus einen Eintrag in „${fallName(zielFall)}" – du prüfst ihn vorher.`
            : 'Echo macht daraus einen Eintrag für deinen Fall – du prüfst ihn vorher.'}
        </p>
      </button>

      {/* Welcher Fall es ist, stand bisher nirgends — die Zuordnung war nach dem einen Mal
          beim Erstellen unsichtbar. Hier ist sie an der Stelle, an der sie wirkt. */}
      <button
        onClick={() => setZielAendern(v => !v)}
        className="mt-1.5 text-[0.68rem] text-brand-muted hover:text-navy"
      >
        {zielAendern ? 'abbrechen' : 'anderen Fall wählen'}
      </button>
      {zielAendern && (
        <div className="mt-2">
          <AnkerFall coupleId={coupleId} caseId={caseId} titel="Wohin die Szenen gehen" />
        </div>
      )}
